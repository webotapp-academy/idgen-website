...
import type {
  DynamicFaqData,
  DynamicFaqHero,
  DynamicFaqHeroShowcase,
  DynamicFaqTopicMatrix,
  DynamicFaqExplorerMatrix,
  DynamicFaqClosingCta,
  DynamicFaqMeta,
  DynamicFaqCategorySection,
  DynamicFaqItem,
} from "@/lib/dynamic-faq-types";

function AdminFaqContent() {
  const searchParams = useSearchParams();
  const initialTab = searchParams.get("tab") || "hero";
  const [data, setData] = useState<DynamicFaqData | null>(null);
  ...
  const fetchPageData = async () => {
    try {
      ...
      if (json.success && json.data) {
        ...
        setSaveError("Failed to load FAQ data");
      }
    } catch (e: any) {
  ...
  const handleSaveSection = async (sectionKey: keyof DynamicFaqData) => {
    if (!data) return;
    try {
      setSaving(true);
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (json.success) {
        setSaveSuccess("Entire FAQ page saved and published successfully!");
  ...
  const handleResetDefaults = async () => {
    if (
      !confirm(
        "Are you sure you want to reset all FAQ sections back to factory defaults? Any custom edits will be reverted."
      )
    ) {
      return;
    }
    ...
    return (
      <div className="flex items-center justify-center min-h-[450px]">
        <div className="flex flex-col items-center gap-3 text-slate-400">
          ...
          <p className="text-xs font-semibold uppercase tracking-wider">Loading FAQ Data...</p>
        </div>
      </div>
    );
  }
  ...
  if (!data) {
    return (
      <div className="p-8 text-center bg-rose-950/20 border border-rose-500/30 rounded-2xl text-rose-300 space-y-3">
        ...
        <h3 className="font-bold text-sm">Failed to load FAQ data</h3>
        <button
  ...
  const totalQuestions = data.explorerMatrix.categories.reduce(
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-16">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900/90 border border-slate-800 p-5 rounded-3xl shadow-xl backdrop-blur-md">
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-400">
            Control all sections, topic highlights, and categorized Q&amp;As of the <code className="text-cyan-300">/faq/</code> page.
          </p>
        </div>
      ...
      <div className="flex flex-wrap gap-1.5 p-1.5 bg-slate-900/80 border border-slate-800 rounded-2xl">
        {[
          ...
          { id: "topics", label: "3. Topic Highlights", icon: Layers },
          { id: "questions", label: `4. FAQ Categories (${totalQuestions})`, icon: HelpCircle },
          { id: "closing", label: "5. Closing CTA & SEO", icon: Lock },
        ].map((tab) => {
          const Icon = tab.icon;
        ...
        <div className="p-6 rounded-3xl bg-slate-900/60 border border-slate-800 space-y-6">
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <div>
              ...
              <p className="text-xs text-slate-400">Configure title text, badges, lede paragraph, and spec strip.</p>
            </div>
            <button
        ...
        <div className="p-6 rounded-3xl bg-slate-900/60 border border-slate-800 space-y-6">
          ...
          <div className="space-y-4">
            <h3 className="text-xs font-bold text-slate-300 uppercase tracking-wider">
              Pillars ({data.heroShowcase.pillars.length})
            </h3>
            <div className="space-y-4">
              {data.heroShowcase.pillars.map((pillar, pIdx) => (
                ...
                >
                  ...
                  {/* Pricing Items (if pricing pillar) */}
                  {pillar.pricingItems && (
                    <div className="space-y-2">
                      <label className="text-[10px] font-mono text-slate-400 block">
                        Pricing Metric Tiles (4 Tiles)
                      </label>
                      <div className="grid gap-2 sm:grid-cols-4">
                        {pillar.pricingItems.map((pItem, iIdx) => (
                          <div key={iIdx} className="p-2 rounded-lg bg-slate-900 border border-slate-800 space-y-1">
                            <input
                              type="text"
                              placeholder="Label"
                              value={pItem.label}
                              onChange={(e) => {
                                const updated = [...data.heroShowcase.pillars];
                                updated[pIdx].pricingItems![iIdx].label = e.target.value;
                                setData({ ...data, heroShowcase: { ...data.heroShowcase, pillars: updated } });
                              }}
                              className="w-full bg-slate-950 border border-slate-800 rounded px-1.5 py-0.5 text-[11px] text-white"
                            />
                            <div className="flex items-center gap-1">
                              <input
                                ...
                                onChange={(e) => {
                                  const updated = [...data.heroShowcase.pillars];
                                  updated[pIdx].pricingItems![iIdx].value = e.target.value;
                                  setData({ ...data, heroShowcase: { ...data.heroShowcase, pillars: updated } });
                                }}
                                className="w-1/2 bg-slate-950 border border-slate-800 rounded px-1.5 py-0.5 text-[11px] text-white font-bold"
                              />
                              <input
                                type="text"
                                placeholder="Unit"
                                ...
                                onChange={(e) => {
                                  const updated = [...data.heroShowcase.pillars];
                                  updated[pIdx].pricingItems![iIdx].unit = e.target.value;
                                  setData({ ...data, heroShowcase: { ...data.heroShowcase, pillars: updated } });
                                }}
                                className="w-1/2 bg-slate-950 border border-slate-800 rounded px-1.5 py-0.5 text-[10px] text-slate-400"
                              />
                            </div>
                          </div>
                        ))}
                  ...
                  {/* Checklist Items (if non-pricing pillar) */}
                  {pillar.items && (
                    <div>
      ...
      {/* ═════════════════════════════════════════════════════════════
          TAB 3: TOPIC MATRIX HIGHLIGHTS
          ═════════════════════════════════════════════════════════════ */}
      {activeTab === "topics" && (
        <div className="p-6 rounded-3xl bg-slate-900/60 border border-slate-800 space-y-6">
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <div>
              <h2 className="text-base font-bold text-white">Topic Matrix Highlights</h2>
              <p className="text-xs text-slate-400">
                Manage the quick navigation topic cards that filter questions.
              </p>
            </div>
            <button
              onClick={() => handleSaveSection("topicMatrix")}
              disabled={saving}
              className="flex items-center gap-1.5 px-4 py-1.5 rounded-xl bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-300 border border-cyan-500/30 text-xs font-bold transition"
            >
              <Save className="h-3.5 w-3.5" />
              <span>{savingSection === "topicMatrix" ? "Saving..." : "Save Section"}</span>
            </button>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <div>
              ...
              <input
                type="text"
                value={data.topicMatrix.eyebrow}
                onChange={(e) =>
                  setData({
                    ...data,
                    topicMatrix: { ...data.topicMatrix, eyebrow: e.target.value },
                  })
                }
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-white"
              />
            ...
            <div>
              ...
              <input
                type="text"
                value={data.topicMatrix.title}
                onChange={(e) =>
                  setData({
                    ...data,
                    topicMatrix: { ...data.topicMatrix, title: e.target.value },
                  })
                }
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-white"
              />
            ...
            <div>
              ...
              <input
                type="text"
                value={data.topicMatrix.badgeText}
                onChange={(e) =>
                  setData({
                    ...data,
                    topicMatrix: { ...data.topicMatrix, badgeText: e.target.value },
                  })
                }
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-white"
              />
          ...
          <div className="space-y-3">
            <h3 className="text-xs font-bold text-slate-300 uppercase tracking-wider">
              Topic Cards ({data.topicMatrix.topics.length})
            </h3>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {data.topicMatrix.topics.map((t, idx) => (
                <div key={idx} className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-2.5">
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-xs text-cyan-400 font-bold">Topic #{idx + 1}</span>
                    <input
                      type="text"
                      value={t.category}
                      placeholder="Category"
                      onChange={(e) => {
                        const updated = [...data.topicMatrix.topics];
                        updated[idx].category = e.target.value;
                        setData({ ...data, topicMatrix: { ...data.topicMatrix, topics: updated } });
                      }}
                  ...
                  <div className="grid gap-2 grid-cols-2">
                    <div>
                      <label className="text-[10px] font-mono text-slate-400">Link Text</label>
                      <input
                        type="text"
                        value={t.linkText}
                        onChange={(e) => {
                    ...
                    <div>
                      <label className="text-[10px] font-mono text-slate-400">Target Category Name</label>
                      <input
                        type="text"
                        value={t.targetCategory}
                        onChange={(e) => {
                          const updated = [...data.topicMatrix.topics];
      ...
      {/* ═════════════════════════════════════════════════════════════
          TAB 4: FAQ EXPLORER CATEGORIES & QUESTIONS
          ═════════════════════════════════════════════════════════════ */}
      {activeTab === "questions" && (
        <div className="p-6 rounded-3xl bg-slate-900/60 border border-slate-800 space-y-6">
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <div>
              <h2 className="text-base font-bold text-white">FAQ Categories &amp; Questions</h2>
              <p className="text-xs text-slate-400">
                Manage all {data.explorerMatrix.categories.length} categories and {totalQuestions} questions with answers.
              </p>
            </div>
            <button
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <h3 className="text-xs font-bold text-slate-300 uppercase tracking-wider">
                Question Categories ({data.explorerMatrix.categories.length})
              </h3>
              <button
                onClick={() => {
                  const newCat: DynamicFaqCategorySection = {
                    name: "New Category",
                    badge: "Topic Badge",
                    faqs: [{ q: "New Question?", a: "Answer text here..." }],
                  };
                  setData({
                    ...data,
                    explorerMatrix: {
                      ...data.explorerMatrix,
                      categories: [...data.explorerMatrix.categories, newCat],
                    },
                  });
                }}
            ...
            <div className="space-y-6">
              {data.explorerMatrix.categories.map((category, cIdx) => (
                <div
                ...
                >
                  <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        value={category.name}
                        onChange={(e) => {
                          const updated = [...data.explorerMatrix.categories];
                          updated[cIdx].name = e.target.value;
                          setData({
                            ...data,
                            explorerMatrix: { ...data.explorerMatrix, categories: updated },
                          });
                        }}
                        className="bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1 text-xs text-white font-bold"
                      />
                      <input
                        type="text"
                    ...
                    <button
                      onClick={() => {
                        const updated = data.explorerMatrix.categories.filter((_, i) => i !== cIdx);
                        setData({
                          ...data,
                          explorerMatrix: { ...data.explorerMatrix, categories: updated },
                        });
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div>
                      <label className="text-[10px] font-mono text-slate-400">Related Service Link</label>
                      <input
                        type="text"
                        placeholder="/student-id-card-printing/"
                        value={category.relatedHref || ""}
                        onChange={(e) => {
                          const updated = [...data.explorerMatrix.categories];
                          updated[cIdx].relatedHref = e.target.value || undefined;
                          setData({
                            ...data,
                            explorerMatrix: { ...data.explorerMatrix, categories: updated },
                          });
                    ...
                    <div>
                      <label className="text-[10px] font-mono text-slate-400">Related Service Label</label>
                      <input
                        type="text"
                        placeholder="Explore Student ID Cards"
                        value={category.relatedLabel || ""}
                        onChange={(e) => {
                          const updated = [...data.explorerMatrix.categories];
                          updated[cIdx].relatedLabel = e.target.value || undefined;
                          setData({
                            ...data,
                            explorerMatrix: { ...data.explorerMatrix, categories: updated },
                          });
                  ...
                  <div className="space-y-3 pt-2">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        onClick={() => {
                          const updated = [...data.explorerMatrix.categories];
                          updated[cIdx].faqs.push({ q: "New Question?", a: "New answer text..." });
                          setData({
                            ...data,
                            explorerMatrix: { ...data.explorerMatrix, categories: updated },
                          });
                    ...
                    <div className="space-y-3">
                      {category.faqs.map((faq, fIdx) => (
                        <div
                          key={fIdx}
                          className="p-3.5 rounded-xl bg-slate-900 border border-slate-800/80 space-y-2"
                        >
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                const updated = [...data.explorerMatrix.categories];
                                updated[cIdx].faqs[fIdx].q = e.target.value;
                                setData({
                                  ...data,
                                  explorerMatrix: { ...data.explorerMatrix, categories: updated },
                                });
                            ...
                            <button
                              onClick={() => {
                                const updated = [...data.explorerMatrix.categories];
                                updated[cIdx].faqs = updated[cIdx].faqs.filter((_, i) => i !== fIdx);
                                setData({
                                  ...data,
                                  explorerMatrix: { ...data.explorerMatrix, categories: updated },
                                });
                          ...
                          <div>
                            <textarea
                              ...
                              onChange={(e) => {
                                const updated = [...data.explorerMatrix.categories];
                                updated[cIdx].faqs[fIdx].a = e.target.value;
                                setData({
                                  ...data,
                                  explorerMatrix: { ...data.explorerMatrix, categories: updated },
                                });
                              }}
                              className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2 text-xs text-slate-300 font-normal"
                            />
                          </div>
                        </div>
                      ))}
    ...
    <Suspense
      fallback={
        <div className="p-8 text-center text-slate-500 text-xs font-semibold">
          Loading FAQ Admin Control Panel...
        </div>
      }
    >
      <AdminFaqContent />
