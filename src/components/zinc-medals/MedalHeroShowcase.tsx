...
type MedalMode = "gold" | "silver" | "bronze" | "cutout";

export function MedalHeroShowcase() {
  const [activeMode, setActiveMode] = useState<MedalMode>("gold");
  ...
  const modes = {
    gold: {
      ...
      badge: "Antique Gold",
      material: "Heavy-Density Eco Zinc Alloy",
      plating: "Antique Gold Electroplate + Hand Buffing",
      ribbon: "25mm Sublimation Full-Color Satin Ribbon",
      image: "/images/Zinc Medal/IMG_20260213_100040.jpg.jpeg",
      alt: "Custom High-Relief Zinc Gold Medal with Ribbon",
    },
    silver: {
      code: "ZNC-03",
      title: "Antique Silver Institutional Medal",
      badge: "Antique Silver",
      material: "Precision Die-Cast Zinc Alloy",
      plating: "Antique Silver / Nickel Electroplate",
      ribbon: "Custom Branded V-Neck Satin Ribbon",
    ...
    bronze: {
      ...
      badge: "Antique Bronze",
      material: "Solid Die-Molded Zinc Alloy",
      plating: "Antique Copper / Bronze Treatment",
      ribbon: "Heavy-Duty Heat-Transfer Printed Ribbon",
      image: "/images/Zinc Medal/20dfd541-e04b-4fc4-944f-5127c3b4ec2e.jpg",
      alt: "Bronze Marathon & Event Medallion",
    },
    cutout: {
      code: "ZNC-04",
      title: "Custom 3D Cutout Sports Medal",
      badge: "Custom Cutout",
      material: "CNC Milled Custom Zinc Alloy Mold",
      plating: "Multi-Tone Plating & Soft Enamel Fill",
      ribbon: "30mm Wide Double-Sided Sublimated Ribbon",
    ...
    <div className="flex flex-col h-full justify-between gap-3.5">
      ...
      >
          ...
          >
            ...
            <div className="mt-3 w-full rounded-2xl bg-white/90 dark:bg-slate-900/90 p-3 border border-slate-200/80 dark:border-slate-800 shadow-sm flex items-center justify-between text-xs font-semibold">
              <span className="text-slate-600 dark:text-slate-300 truncate max-w-[70%]">
                <strong className="text-slate-900 dark:text-white">Ribbon: </strong>
                {current.ribbon}
              </span>
        ...
        <div className="relative z-20 p-2.5 sm:p-3 border-t border-slate-200/90 dark:border-slate-800 bg-slate-50/90 dark:bg-slate-900/90 backdrop-blur-md">
          <div className="grid grid-cols-4 gap-1.5 sm:gap-2">
            {[
              { id: "gold", label: "Antique Gold", sub: "1st Place" },
              { id: "silver", label: "Antique Silver", sub: "2nd Place" },
              { id: "bronze", label: "Antique Bronze", sub: "3rd Place" },
              { id: "cutout", label: "Custom Cutout", sub: "3D Mold" },
            ].map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveMode(tab.id as MedalMode)}
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="rounded-2xl border border-slate-200/90 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-900/80 p-2.5 text-center shadow-2xs">
          ...
          <span className="text-xs font-black text-slate-900 dark:text-white">Eco Zinc Alloy</span>
        </div>
        <div className="rounded-2xl border border-slate-200/90 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-900/80 p-2.5 text-center shadow-2xs">
          <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase block">Plating</span>
          <span className="text-xs font-black text-[#009fe3] dark:text-cyan-400">Gold / Silver / Bronze</span>
        </div>
        <div className="rounded-2xl border border-slate-200/90 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-900/80 p-2.5 text-center shadow-2xs">
          <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase block">Ribbon</span>
          <span className="text-xs font-black text-emerald-600 dark:text-emerald-400">Full-Color Satin</span>
        </div>
      </div>
    </div>
  );
