...
export function HookApplicationsGrid({
  ...
  const activeUses =
    data?.uses && data.uses.length > 0 ? data.uses : hookUsesList;
  const activeCommonUses =
    data?.commonUses && data.commonUses.length > 0
      ? data.commonUses
      : commonWearableUses;

  const badgeText = data?.badge || "Applications";
  const titleText = data?.title || "ID Card Hook Uses";
  const ledeText =
    data?.lede ||
    "ID card hooks can be used with different identification applications.";
  const subtitleText = data?.subtitle || "Wearable Identification Systems";
  const commonTitleText =
    data?.commonUsesTitle ||
    "Hooks are commonly used as part of wearable identification systems for:";

  return (
    <section className="mt-16 sm:mt-20 scroll-mt-28" id="hook-applications">
            ...
            >
              <div
                className={`absolute inset-0 bg-gradient-to-br ${
                  item.gradient || "from-sky-500/10 via-blue-500/5 to-transparent"
              ...
              <div className="relative z-10 space-y-4">
                <div className="flex items-center justify-between">
                  <div
                    className={`flex h-11 w-11 items-center justify-center rounded-2xl bg-white dark:bg-slate-800 shadow-md ${
                      item.accentColor || "text-[#009fe3]"
                    }`}
                  >
