  ...
  return (
    <section aria-labelledby="local-presence-heading" className="relative">
      ...
      <div className="relative rounded-[2.5rem] border border-slate-200/90 dark:border-white/10 bg-gradient-to-b from-white via-slate-50/50 to-white dark:from-[#0b1320] dark:via-[#0e1726] dark:to-[#070d18] p-6 sm:p-8 lg:p-10 shadow-2xl shadow-slate-900/5 dark:shadow-black/60 overflow-hidden">
        ...
        {/* Dynamic Key Localities & Coverage Ribbon */}
        {city.nearbyAreas && city.nearbyAreas.length > 0 ? (
          <div className="relative z-10 mt-8 pt-6 border-t border-slate-200/80 dark:border-white/10">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
              ...
              <span className="text-[11px] text-muted">
                {city.deliveryRoutesSubtitle || `Rapid fulfillment across all major institutional & commercial zones in ${city.name}`}
              </span>
            </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {city.nearbyAreas.map((area) => (
                <span
                  key={area}
                  className="rounded-full bg-white dark:bg-white/5 border border-slate-200/90 dark:border-white/10 px-3 py-1 text-[11px] font-medium text-foreground hover:border-accent hover:text-accent transition shadow-2xs"
                >
                  {area}
                </span>
              ))}
          ...
          <div className="relative z-10 mt-8 pt-6 border-t border-slate-200/80 dark:border-white/10">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-xs font-bold uppercase tracking-wider text-foreground">
                  {city?.deliveryRoutesTitle || `Active Doorstep Fulfillment Zones in ${cityName}:`}
                </h4>
              </div>
              <span className="text-[11px] text-muted">
                {city?.deliveryRoutesSubtitle || `Express courier and direct door-to-door delivery active across ${cityName}, ${stateName}`}
              </span>
            </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {[
                `${cityName} Central`,
                `${cityName} Educational District`,
                `${cityName} Commercial Area`,
                `${cityName} Industrial Belt`,
                `${cityName} Institutional Hub`,
                `${cityName} Bypass Route`,
              ].map((area) => (
                <span
                  key={area}
                  className="rounded-full bg-white dark:bg-white/5 border border-slate-200/90 dark:border-white/10 px-3 py-1 text-[11px] font-medium text-foreground hover:border-accent hover:text-accent transition shadow-2xs"
